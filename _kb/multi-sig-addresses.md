...
When you're confident the transaction is good, you can send the transaction hex to the first owner of a private keys. He should inspect the transaction with `decoderawtransaction` and if he approves he can sign the transaction with this command:

{% highlight bash %}
./nud -unit=B signrawtransaction TRANSACTION_HEX
{% endhighlight %}

He will get the hex string of the partially signed transaction. He must send it to the next owner, and so on. The transaction is fully signed when `signrawtransaction` returns with `complete` set to `true`.
